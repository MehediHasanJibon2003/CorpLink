    ...
    <div className="space-y-12 md:space-y-16 pb-20 animate-in fade-in duration-700">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-[3rem] p-6 md:p-16 border-2 border-slate-100 dark:border-white/5 shadow-sm">
        <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-12">
           ...
           <div>
              <h3 className="text-2xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Draft <br className="hidden md:block" /> Proposal</h3>
              <p className="text-[10px] md:text-body font-black text-slate-400 uppercase tracking-widest mt-1">Strategic Agreement</p>
           </div>
        </div>

        {error && <p className="p-4 md:p-6 bg-red-50 text-red-600 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-[10px] mb-6 md:mb-8">{error}</p>}
        {message && <p className="p-4 md:p-6 bg-emerald-50 text-emerald-600 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-[10px] mb-6 md:mb-8">{message}</p>}

        <form onSubmit={handleSendProposal} className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
          <div className="space-y-2 md:space-y-3">
            <label className="text-[9px] md:text-label font-black text-slate-400 uppercase tracking-widest px-2">Collaborator</label>
            <select
          ...
          <div className="space-y-2 md:space-y-3">
            <label className="text-[9px] md:text-label font-black text-slate-400 uppercase tracking-widest px-2">Agreement Tier</label>
            <select
          ...
          <div className="md:col-span-2 space-y-2 md:space-y-3">
            <label className="text-[9px] md:text-label font-black text-slate-400 uppercase tracking-widest px-2">Operational Title</label>
            <input
              type="text"
          ...
          <div className="md:col-span-2 space-y-2 md:space-y-3">
            <label className="text-[9px] md:text-label font-black text-slate-400 uppercase tracking-widest px-2">Scope of Operations</label>
            <textarea
          ...
          >
            {partners.length === 0 ? "Establish Connection" : "Transmit Proposal"}
          </button>
        </form>
      </div>
      ...
      <div className="space-y-8 md:space-y-10">
         <div className="flex items-center gap-4 md:gap-6">
            ...
            <h3 className="text-xl md:text-heading-1 font-black text-slate-900 dark:text-white tracking-tight uppercase">Protocol Log</h3>
         </div>
         ...
         <div className="grid grid-cols-1 gap-6 md:gap-8">
            {proposals.length === 0 ? (
              <div className="py-12 md:py-20 text-center bg-slate-50 dark:bg-slate-900/20 rounded-2xl md:rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800">
                 <p className="text-[14px] md:text-heading-2 font-black text-slate-300 uppercase tracking-widest">No active protocols.</p>
              </div>
            ) : (
              proposals.map(prop => {
                ...
                return (
                  <div key={prop.id} className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-[3rem] p-6 md:p-10 border-2 border-slate-100 dark:border-white/5 flex flex-col gap-6 md:gap-8 group hover:border-blue-500/30 transition-all">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                       <div className="space-y-2 w-full md:w-auto">
                          <div className="flex flex-wrap items-center gap-2 md:gap-3">
                             <span className={`px-3 py-0.5 md:px-4 md:py-1 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest ${isSent ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'}`}>
                          ...
                          <h4 className="text-[16px] md:text-heading-1 font-black text-slate-900 dark:text-white tracking-tight leading-tight uppercase">{prop.title}</h4>
                       </div>
                       <div className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-2 w-fit ${prop.status === 'accepted' ? 'bg-emerald-100 text-emerald-700' : prop.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                          {prop.status === 'accepted' ? <CheckCircle2 className="h-3 w-3 md:h-4 md:w-4" /> : prop.status === 'rejected' ? <XCircle className="h-3 w-3 md:h-4 md:w-4" /> : <Clock className="h-3 w-3 md:h-4 md:w-4" />}
                          {prop.status}
                       </div>
                    </div>
                    ...
                    {!isSent && prop.status === 'pending' && (
                      <div className="flex gap-2 md:gap-4 w-full sm:w-auto">
                        <button onClick={() => handleUpdateStatus(prop.id, 'accepted')} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-3 md:py-5 rounded-lg md:rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-label shadow-lg active:scale-95 transition-all">Authorize</button>
                        <button onClick={() => handleUpdateStatus(prop.id, 'rejected')} className="flex-1 bg-slate-100 dark:bg-slate-700 text-slate-500 py-3 md:py-5 rounded-lg md:rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-label hover:bg-red-500 hover:text-white transition-all">Reject</button>
                      </div>
                    )}
                  </div>
                )
