  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 md:p-8 hover:shadow-xl transition-all duration-300">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-body md:text-heading-2 font-medium text-slate-500 dark:text-slate-400">{title}</p>
          <h3 className="text-heading-1 md:text-5xl lg:text-6xl font-black text-slate-800 dark:text-white mt-2 md:mt-4 tracking-tight">{value}</h3>
        </div>
        {Icon && (
      ...
      {subtitle && <p className="text-label md:text-heading-3 text-slate-400 dark:text-slate-500 mt-3 md:mt-5 font-medium flex items-center gap-1 md:gap-2">{subtitle}</p>}
    </div>
  );
}
