  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
        ...
        <div className="relative z-10">
          ...
          <p className="text-blue-200 text-sm mt-1 max-w-md">
            Here's your productivity snapshot for today. Stay focused and keep
            crushing your goals.
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 md:gap-12">
        {/* Recent Tasks */}
        <div className="lg:col-span-2 border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-3xl md:rounded-[3rem] shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="divide-y-2 divide-slate-100 dark:divide-slate-700/50">
              {recentTasks.map((task) => (
                <div
                  key={task.id}
                  className="flex items-center justify-between px-8 md:px-12 py-6 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-3 ml-6 shrink-0">
                    <span
                      className={`text-xs md:text-sm font-black px-4 py-1.5 rounded-full uppercase tracking-widest ${priorityColor[task.priority] || priorityColor.low}`}
                    >
                      {task.priority || "low"}
                    </span>
                    <span
                      className={`text-xs md:text-sm font-black px-4 py-1.5 rounded-full capitalize tracking-widest ${statusColor[task.status] || statusColor.pending}`}
                    >
            ...
            {loading ? (
              ...
              <div className="divide-y-2 divide-slate-100 dark:divide-slate-700/50">
                {notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`px-8 py-5 ${!n.is_read ? "bg-blue-50/50 dark:bg-blue-900/10" : ""}`}
                  >
                    <p className="text-sm md:text-base font-bold text-slate-700 dark:text-slate-300 leading-relaxed">
                      {n.message}
                    </p>
                    <p className="text-xs md:text-sm font-bold text-slate-400 mt-2 uppercase tracking-wider">
          ...
          <div className="border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-3xl md:rounded-[3rem] shadow-sm p-8 md:p-10">
            ...
            <div className="space-y-4">
              ...
              <QuickLink
                ...
                color="bg-slate-50 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                navigate={navigate}
              />
            </div>
          </div>
