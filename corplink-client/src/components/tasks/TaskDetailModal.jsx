...
function TaskDetailModal({ task, onClose, profile, onUpdate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden animate-in zoom-in-95">
        
        {/* Left Side: Task Info & Workflow */}
        <div className="w-full md:w-[45%] bg-slate-50 border-r border-slate-200 p-6 flex flex-col overflow-y-auto">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 shadow-sm">
            <h4 className="font-semibold text-slate-800 text-xs uppercase tracking-wider mb-3">Workflow Controls</h4>
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-bold">👤</div>
                <span className="font-semibold text-slate-800">{employees.find(e => e.id === task.assigned_to)?.name || "Unassigned"}</span>
              </div>
            </div>
            <div>
              <p className="text-slate-500 font-semibold mb-1">Deadline</p>
              <p className="text-slate-800 font-medium">{task.deadline ? new Date(task.deadline).toLocaleDateString() : "No Deadline set"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-[55%] flex flex-col bg-white">
          <div className="flex border-b border-slate-200">
            <button onClick={() => setActiveTab('comments')} className={`flex-1 py-4 font-semibold text-sm transition border-b-2 ${activeTab === 'comments' ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>Activity & Comments</button>
            <button onClick={() => setActiveTab('attachments')} className={`flex-1 py-4 font-semibold text-sm transition border-b-2 ${activeTab === 'attachments' ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>Files & Links ({attachments.length})</button>
          </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {comments.length === 0 ? (
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`px-4 py-3 rounded-2xl max-w-[85%] text-sm shadow-sm ${isMe ? 'bg-slate-800 text-white rounded-tr-sm' : 'bg-slate-100 text-slate-800 rounded-tl-sm'}`}>
                        {msg.content}
                      </div>
                    </div>
                  )
              ...
              <div className="p-4 border-t border-slate-100 bg-white">
                <form onSubmit={handlePostComment} className="flex gap-2">
                  <input type="text" value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Write an update..." className="flex-1 bg-slate-50 border border-slate-200 rounded-full px-4 py-2 outline-none focus:border-slate-400 text-sm" />
          ...
          {activeTab === 'attachments' && (
            <div className="flex-1 flex flex-col overflow-hidden p-6 gap-6">
              <form onSubmit={handleUploadLink} className="bg-slate-50 p-4 rounded-xl border border-slate-200 border-dashed">
                <h4 className="font-semibold text-slate-700 mb-3 text-sm">Attach new Document / URL</h4>
                <div className="flex flex-col gap-2">
                  <input type="text" value={newName} onChange={e=>setNewName(e.target.value)} placeholder="e.g. Figma Design, Google Doc" className="border border-slate-300 rounded px-3 py-2 text-sm outline-none" />
                  <input type="url" value={newUrl} onChange={e=>setNewUrl(e.target.value)} placeholder="https://..." className="border border-slate-300 rounded px-3 py-2 text-sm outline-none" />
              ...
              <div className="flex-1 overflow-y-auto space-y-3">
                {attachments.map(att => (
                  <a key={att.id} href={att.file_url} target="_blank" rel="noreferrer" className="flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:shadow-md hover:border-blue-300 transition group bg-white">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h5 className="font-semibold text-slate-800 text-sm group-hover:text-blue-600 transition">{att.file_name}</h5>
                        <p className="text-[11px] text-slate-500 flex gap-2">By {att.uploader?.full_name} • {new Date(att.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
