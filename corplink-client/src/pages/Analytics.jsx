...
import { 
  ...
  PieChart, Pie, Cell, LineChart, Line, AreaChart, Area, Radar, RadarChart, 
  PolarGrid, PolarAngleAxis, PolarRadiusAxis 
} from 'recharts';
import { 
  BarChart3, TrendingUp, Users, Target, Loader2, CheckCircle2, 
  MessageSquare,Zap, Award, Layout, MousePointer2 
} from "lucide-react";

const COLORS = ['#2563eb', '#8b5cf6', '#f59e0b', '#10b981', '#6366f1', '#ef4444'];
...
export default function Analytics() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const { data: tasks } = await supabase.from("tasks").select("*, projects(name), profiles:assigned_to(full_name)").eq("company_id", profile.company_id);
      const { data: messages } = await supabase.from("internal_messages").select("created_at, sender_id").gte('created_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString());
      const { data: employees } = await supabase.from("profiles").select("id, full_name, role").eq("company_id", profile.company_id);

      // --- OVERVIEW LOGIC ---
      const finished = tasks?.filter(t => t.status === 'finished' || t.status === 'completed').length || 0;
      const total = tasks?.length || 0;
      
      const overview = {
        velocity: total > 0 ? Math.round((finished / total) * 100) : 0,
        totalTasks: total,
        activeProjects: new Set(tasks?.map(t => t.project_id).filter(Boolean)).size,
        engagementScore: Math.min(100, Math.round((messages?.length || 0) / (employees?.length || 1) * 10))
      };
      ...
      tasks?.forEach(t => {
        ...
        perfMap[name].total++;
        if (t.status === 'finished' || t.status === 'completed') perfMap[name].completed++;
      });
      const performance = Object.values(perfMap).sort((a,b) => b.completed - a.completed).slice(0, 5);
      ...
      const engagement = Object.keys(engagementMap).map(day => ({ day, count: engagementMap[day] }));

      // --- COLLABORATION LOGIC ---
      const collabData = [
        { subject: 'Communication', A: overview.engagementScore, fullMark: 100 },
        { subject: 'Task Speed', A: overview.velocity, fullMark: 100 },
        { subject: 'Project Density', A: Math.min(100, overview.activeProjects * 20), fullMark: 100 },
  ...
  return (
    <AppLayout title="Enterprise Insights" subtitle="Real-time organizational performance monitoring">
      ...
      {activeTab === "overview" && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              {[
                { label: "Mission Velocity", value: `${data.overview.velocity}%`, icon: TrendingUp, color: "text-blue-500" },
                { label: "Total Objectives", value: data.overview.totalTasks, icon: Target, color: "text-purple-500" },
                { label: "Active Sectors", value: data.overview.activeProjects, icon: Layout, color: "text-emerald-500" },
                { label: "Engagement Score", value: `${data.overview.engagementScore}%`, icon: MessageSquare, color: "text-orange-500" },
              ].map((stat, i) => (
              ...
              <div className="h-[400px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data.engagement}>
                       ...
                       <Area type="monotone" dataKey="count" stroke="#3b82f6" strokeWidth={4} fill="url(#colorEngagement)" />
                    </AreaChart>
                 </ResponsiveContainer>
              </div>
           </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
           ...
           <div className="bg-white dark:bg-slate-800 p-12 rounded-[3rem] border-2 border-slate-100 dark:border-white/5 shadow-sm">
              ...
              <div className="h-[400px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                       <Pie data={data.performance} dataKey="completed" cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={5}>
                          {data.performance.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                       </Pie>
           ...
           <div className="h-[500px] w-full max-w-4xl">
              <ResponsiveContainer width="100%" height="100%">
                 <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data.collaboration}>
                    ...
                    <PolarAngleAxis dataKey="subject" tick={{ fontSize: 12, fontWeight: 'black', fill: '#94a3b8' }} />
                    <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
                    <Radar name="Synergy" dataKey="A" stroke="#2563eb" fill="#3b82f6" fillOpacity={0.5} />
                 </RadarChart>
              </ResponsiveContainer>
           </div>
