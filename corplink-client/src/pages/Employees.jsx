    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 md:mb-12">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border-2 border-slate-100 dark:border-violet-500/10 shadow-sm">
          <p className="text-label font-black uppercase tracking-widest text-slate-500 mb-2">
            Total Directory
          </p>
          <h3 className="text-heading-1 font-black text-slate-900 dark:text-white">
            {employees.length}
          </h3>
        </div>
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border-2 border-slate-100 dark:border-violet-500/10 shadow-sm">
          ...
          <h3 className="text-heading-1 font-black text-emerald-500">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border-2 border-slate-100 dark:border-violet-500/10 shadow-sm">
          ...
          <h3 className="text-heading-1 font-black text-blue-500">
            {departments.length}
          </h3>
        </div>
        <div className="bg-blue-600 p-8 rounded-3xl shadow-xl shadow-blue-500/20 flex flex-col justify-between">
          <p className="text-label font-black uppercase tracking-widest text-blue-100">
            Invite Code
          </p>
          <div className="flex items-center justify-between mt-2">
            <code className="text-heading-2 font-black text-white">
            ...
            <button
              ...
              className="text-[10px] font-black uppercase bg-white/20 px-3 py-1 rounded-lg text-white"
            >
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
          ...
          <div className="sticky top-8 bg-white dark:bg-slate-800 p-8 md:p-10 rounded-[2.5rem] border-2 border-slate-100 dark:border-violet-500/15 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-violet-500/10 rounded-2xl px-6 py-4 text-body font-bold focus:border-blue-500 outline-none transition-all"
              />
              <input
                type="email"
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  ...
                  className="bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-violet-500/10 rounded-2xl px-4 py-4 text-label font-black uppercase"
                >
              ...
              <input
                type="text"
                placeholder="Designation (e.g. Senior Designer)"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-violet-500/10 rounded-2xl px-6 py-4 text-body font-bold outline-none"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-label tracking-widest shadow-xl shadow-blue-500/20 hover:scale-[1.02] active:scale-95 transition-all"
              >
                {loading
                  ? "Syncing..."
                  : editingId
            ...
            {error && (
              <p className="mt-4 text-red-500 text-label font-bold bg-red-50 p-4 rounded-xl">
                {error}
              </p>
            )}
            {message && (
              <p className="mt-4 text-emerald-500 text-label font-bold bg-emerald-50 p-4 rounded-xl">
                {message}
              </p>
            )}
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-3xl border-2 border-slate-100 dark:border-violet-500/15 flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-900/50 rounded-2xl pl-12 pr-4 py-3 text-body font-bold outline-none border-2 border-transparent focus:border-violet-500/30"
              />
            </div>
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <select
                ...
                className="flex-1 md:w-48 bg-slate-50 dark:bg-slate-900/50 rounded-xl px-4 py-3 text-[10px] font-black uppercase tracking-widest outline-none"
              >
          ...
          <div className="space-y-4">
            {filteredEmployees.length === 0 ? (
              <div className="py-20 text-center bg-white dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200">
              ...
              filteredEmployees.map((emp) => {
                ...
                return (
                  <div
                    key={emp.id}
                    className="group bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2.5rem] border-2 border-slate-100 dark:border-violet-500/10 shadow-sm hover:border-blue-500/30 transition-all"
                  >
                    <div className="flex flex-col md:flex-row justify-between gap-6">
                      <div className="flex gap-5 md:gap-6">
                        <div
                          className={`w-14 h-14 md:w-20 md:h-20 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center font-black text-heading-2 md:text-heading-1 text-white shadow-lg shrink-0 ${emp.onboarded ? "bg-gradient-to-br from-blue-600 to-indigo-600" : "bg-slate-200 text-slate-400"}`}
                        >
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            <h4 className="text-heading-3 md:text-heading-1 font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                              ...
                              {emp.onboarded && (
                                <div
                                  className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-pulse"
                          ...
                          <div className="flex flex-col gap-1.5">
                            <p className="text-label md:text-body font-bold text-slate-500 flex items-center gap-2">
                      ...
                      <div className="flex md:flex-col lg:flex-row items-center gap-4 md:gap-6 bg-slate-50 dark:bg-white/5 p-4 md:p-6 rounded-3xl border-2 border-slate-100 dark:border-white/5">
                        <div className="text-center">
                          ...
                          <p className="text-heading-2 font-black text-slate-800 dark:text-white">
                            {stats.total}
                          </p>
                        </div>
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-heading-2 font-black text-emerald-600">
                            {stats.completed}
                          </p>
                        </div>
                        <div className="w-px h-8 bg-slate-200 dark:bg-white/10 hidden lg:block" />
                        <div className="text-center">
                          ...
                          <p className="text-heading-2 font-black text-blue-600">
                            {stats.inProgress}
                          </p>
                        </div>
                      </div>
                    ...
                    {/* Footer Actions */}
                    <div className="mt-8 pt-6 border-t-2 border-slate-50 dark:border-white/5 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 text-slate-400 hover:text-blue-500 transition-all"
                        >
