...
function Employees() {
  ...
  const fetchEmployees = async () => {
    ...
    tasks?.forEach(t => {
      ...
      if (!stats[t.assigned_to]) stats[t.assigned_to] = { total: 0, completed: 0, inProgress: 0 }
  ...
  const resetForm = () => {
    setForm({
      name: "", email: "", department_id: "", role: "employee",
    ...
    try {
      if (editingId) {
        const { error } = await supabase.from("employees").update({
          ...
          department_id: form.department_id || null, role: form.role,
          designation: form.designation, joining_date: form.joining_date
        }).eq("id", editingId)
        if (error) throw error
        
        await supabase.from("activity_logs").insert([{
          company_id: profile.company_id, user_id: user.id,
          action: `Updated Employee Profile: ${form.name}`, entity: "employee"
        }])
        ...
        await supabase.from("activity_logs").insert([{
          company_id: profile.company_id, user_id: user.id,
          action: `Added New Employee: ${form.name}`, entity: "employee"
        }])
  ...
  return (
    <AppLayout title="Employee Management" subtitle="Systematic directory, performance tracking, and onboarding.">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select value={form.role} onChange={e=>setForm({...form, role: e.target.value})} className="bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-violet-500/10 rounded-2xl px-4 py-4 text-label font-black uppercase">
                  ...
                  <option value="restricted">Intern / Trainee (Restricted)</option>
                </select>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-3xl border-2 border-slate-100 dark:border-violet-500/15 flex flex-col md:flex-row gap-4 items-center">
             ...
             <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <select value={deptFilter} onChange={e=>setDeptFilter(e.target.value)} className="flex-1 md:w-48 bg-slate-50 dark:bg-slate-900/50 rounded-xl px-4 py-3 text-[10px] font-black uppercase tracking-widest outline-none">
          ...
          <div className="space-y-4">
             {filteredEmployees.length === 0 ? (
               <div className="py-20 text-center bg-white dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200">
                  <p className="text-slate-400 font-black uppercase tracking-[0.2em]">No employees found</p>
               </div>
             ) : filteredEmployees.map(emp => {
               const stats = taskStats[emp.id] || { total: 0, completed: 0, inProgress: 0 }
                 ...
                 <div key={emp.id} className="group bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2.5rem] border-2 border-slate-100 dark:border-violet-500/10 shadow-sm hover:border-blue-500/30 transition-all">
                          ...
                          <div>
                             <div className="flex flex-wrap items-center gap-2 mb-1">
                                ...
                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                  ...
                                  emp.role === 'restricted' ? 'bg-slate-100 text-slate-500' :
                    ...
                    <div className="mt-8 pt-6 border-t-2 border-slate-50 dark:border-white/5 flex items-center justify-between">
                       <div className="flex items-center gap-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          <Calendar className="h-3.5 w-3.5" /> Joined {new Date(emp.joining_date).toLocaleDateString()}
                       </div>
