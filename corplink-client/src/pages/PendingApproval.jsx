    ...
    <div className="h-screen w-screen flex flex-col bg-slate-50 dark:bg-[#05030f] relative overflow-hidden">
      ...
      <div className="w-full p-6 flex justify-between items-center relative z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-black text-slate-900 dark:text-white tracking-tight">CorpLink</span>
        </div>
        
        <button 
          onClick={logout}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
        >
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="max-w-md w-full bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-8 shadow-2xl text-center">
          ...
          <div className="bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/5 rounded-2xl p-4 mb-8 text-left">
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Workspace Details</p>
            <p className="text-sm font-semibold text-slate-900 dark:text-white">{profile?.companies?.name || "Unknown Corporate"}</p>
            <p className="text-xs text-slate-500 mt-1">Status: <span className={isRejected ? "text-red-500 font-bold" : "text-amber-500 font-bold"}>{profile?.companies?.status?.toUpperCase()}</span></p>
          </div>

          {!isRejected && (
            <button onClick={() => window.location.reload()} className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 transition-all shadow-lg hover:shadow-blue-600/30">
              Refresh Status
