    ...
    <AppLayout title="System Configuration" subtitle="Manage enterprise parameters and personal preferences">
      <div className="bg-white dark:bg-slate-800 rounded-3xl md:rounded-[3rem] shadow-sm border-2 border-slate-100 dark:border-white/5 flex flex-col xl:flex-row min-h-[850px] overflow-hidden">
        ...
        <div className="w-full xl:w-96 border-b-2 xl:border-b-0 xl:border-r-2 border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/30 p-10 flex flex-col">
          <h3 className="text-body font-black text-slate-400 uppercase tracking-[0.3em] mb-12">Settings Hub</h3>
          <nav className="space-y-4">
            {tabs.map((tab) => (
              <button
                key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-5 w-full px-8 py-6 rounded-3xl font-black uppercase tracking-widest text-body transition-all ${
          ...
          <div className="mt-auto pt-10">
             <div className="p-6 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl text-white shadow-lg">
                <p className="text-label font-black uppercase tracking-widest opacity-60">Corporate License</p>
                <p className="text-heading-2 font-black mt-1">Enterprise Plan</p>
        ...
        <div className="flex-1 p-12 md:p-20 overflow-y-auto custom-scrollbar">
          {message && (
            <div className="mb-10 p-6 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 border-2 border-emerald-100 dark:border-emerald-800/50 rounded-2xl font-black uppercase text-label tracking-widest flex items-center gap-4 animate-in fade-in slide-in-from-top-4">
          ...
          {activeTab === 'profile' && (
            <div className="max-w-4xl animate-in fade-in slide-in-from-right-4 duration-500">
              <h3 className="text-heading-1 font-black text-slate-900 dark:text-white mb-16 tracking-tight flex items-center gap-4">
              ...
              <div className="space-y-12">
                <div className="flex items-center gap-10">
                   ...
                   <button className="px-10 py-5 bg-slate-100 dark:bg-white/5 rounded-2xl font-black uppercase tracking-widest text-body hover:bg-slate-200 transition-all">Change Avatar</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="md:col-span-2">
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block">Full Name</label>
                    <input type="text" value={userForm.full_name} onChange={e => setUserForm({full_name: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-white/5 rounded-2xl px-8 py-6 text-heading-2 font-black outline-none focus:border-blue-500 transition-all" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block">Corporate Email</label>
                    <input type="text" value={user?.email} disabled className="w-full bg-slate-100 dark:bg-slate-900/50 border-2 border-transparent rounded-2xl px-8 py-6 text-heading-2 font-black opacity-50 cursor-not-allowed" />
                  </div>
                </div>
          ...
          {activeTab === 'org' && (
            <div className="max-w-4xl animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center justify-between mb-16">
                 <h3 className="text-heading-1 font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-4">
              ...
              <div className="space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="md:col-span-2">
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block">Company Name</label>
                    <input type="text" value={orgForm.name} onChange={e => setOrgForm({...orgForm, name: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-white/5 rounded-2xl px-8 py-6 text-heading-2 font-black outline-none focus:border-blue-500 transition-all" />
                  </div>
                  <div>
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block text-emerald-500">Industry</label>
                    <div className="relative">
                      <Briefcase className="absolute left-6 top-1/2 -translate-y-1/2 h-6 w-6 text-slate-400" />
                      <input type="text" value={orgForm.industry} onChange={e => setOrgForm({...orgForm, industry: e.target.value})} placeholder="e.g. Technology" className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-white/5 rounded-2xl pl-16 pr-8 py-6 text-heading-2 font-black outline-none focus:border-blue-500 transition-all" />
                    </div>
                  </div>
                  <div>
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block text-blue-500">Website</label>
                    <div className="relative">
                  ...
                  <div className="md:col-span-2">
                    <label className="text-label font-black text-slate-400 uppercase tracking-widest mb-3 block text-rose-500">Headquarters</label>
                    <div className="relative">
            ...
            <div className="max-w-4xl animate-in fade-in slide-in-from-right-4 duration-500">
               ...
               <div className="space-y-16">
                  <div className="p-10 bg-slate-50 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center text-center">
                     ...
                     <p className="text-heading-2 font-black text-slate-800 dark:text-white uppercase tracking-tight">Upload Corporate Logo</p>
                     <p className="text-body text-slate-500 font-bold mt-2 uppercase tracking-widest">Transparent PNG recommended (512x512)</p>
                  </div>

                  <div>
                     <label className="text-label font-black text-slate-400 uppercase tracking-[0.2em] mb-8 block">Brand Primary Color</label>
          ...
          {(activeTab === 'notifications' || activeTab === 'security') && (
            <div className="h-full flex flex-col items-center justify-center text-center py-20">
               ...
               <h3 className="text-heading-1 font-black text-slate-800 dark:text-white uppercase tracking-tight mb-4">Under Strategic Review</h3>
               <p className="text-heading-2 text-slate-500 dark:text-slate-400 font-medium max-w-md">This high-security section is currently undergoing a structural audit.</p>
            </div>
          )}
        </div>
      </div>
