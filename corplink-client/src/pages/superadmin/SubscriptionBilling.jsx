      ...
      <div className="relative z-10">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[800px] lg:min-w-full">
                ...
                <tbody className="divide-y-2 divide-slate-100 dark:divide-violet-500/5">
                  {loading ? (
                    <tr><td colSpan={4} className="py-20 text-center animate-pulse font-black text-slate-400">Loading...</td></tr>
                  ) : subs.map(sub => (
                    <tr key={sub.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                      <td className="px-8 py-6 md:py-8">
                        <p className="text-base md:text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{sub.companies?.name}</p>
                        <p className="text-[10px] font-bold text-slate-500 truncate">{sub.companies?.email}</p>
                      </td>
        ...
        {activeTab === "plans" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {plans.map(plan => (
              <div key={plan.id} className="rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-10 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 relative overflow-hidden group">
                <h3 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight mb-2">{plan.name}</h3>
                <div className="flex items-baseline gap-2 mb-8">
                  <span className="text-4xl md:text-5xl font-black text-violet-600">${plan.price}</span>
                ...
                <button className="w-full py-4 rounded-2xl bg-slate-100 dark:bg-white/5 font-black uppercase text-[10px] md:text-xs tracking-widest text-slate-600 dark:text-violet-400">Edit Plan</button>
              </div>
            ))}
            <button className="rounded-[2.5rem] md:rounded-[3rem] p-10 border-2 border-dashed border-slate-200 dark:border-violet-500/20 flex flex-col items-center justify-center gap-4 text-slate-400 hover:text-violet-500 transition-all min-h-[300px]">
