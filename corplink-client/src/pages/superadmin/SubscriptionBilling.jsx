      ...
      <div className="flex gap-3 md:gap-6 mb-8 md:mb-12 overflow-x-auto pb-2 no-scrollbar">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 md:gap-3 px-6 md:px-8 py-4 md:py-5 rounded-2xl md:rounded-[2rem] font-black uppercase text-[10px] md:text-label tracking-widest transition-all border-2 shrink-0 ${
              activeTab === tab.id 
                  ...
                  {loading ? (
                    ...
                    <tr key={sub.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] group">
                      <td className="px-8 py-6 md:py-8">
                        <p className="text-body md:text-heading-2 font-black text-slate-900 dark:text-white uppercase tracking-tight">{sub.companies?.name || 'Unknown Company'}</p>
                        <p className="text-[10px] font-bold text-slate-500 truncate">ID: {sub.company_id.substring(0, 8)}...</p>
                      </td>
                      <td className="px-8 py-6 md:py-8 font-black uppercase text-[10px] md:text-label text-violet-600 dark:text-violet-400">{sub.plan}</td>
                      <td className="px-8 py-6 md:py-8">
        ...
        {activeTab === "plans" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {plans.map(plan => (
              ...
              >
                <h3 className="text-heading-1 md:text-heading-1 font-black text-slate-900 dark:text-white uppercase tracking-tight mb-2 w-full">{plan.name}</h3>
                ...
                <div className="space-y-3 mb-10 w-full flex flex-col items-center">
                  {plan.features?.map((f, idx) => (
                    <div key={idx} className="flex items-center gap-3 text-label md:text-body font-bold text-slate-600 dark:text-violet-300">
                ...
                <div className="flex gap-4 mt-10 w-full">
                  <button 
                    ...
                    className="flex-1 py-4 rounded-2xl bg-slate-100 dark:bg-white/5 font-black uppercase text-[10px] md:text-label tracking-widest text-slate-600 dark:text-violet-400 transition-all flex items-center justify-center gap-2 hover:text-white"
            ...
            >
              ...
              <span className="font-black uppercase tracking-widest text-[10px] md:text-body">Add New Plan</span>
            </button>
          </div>
        )}
                      ...
                      <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] group">
                        <td className="px-8 py-6 md:py-8">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-[10px] md:text-body">{inv.invoice_number}</p>
                          </div>
                        </td>
                        <td className="px-8 py-6 md:py-8">
                          <p className="font-black text-slate-900 dark:text-white uppercase text-[10px] md:text-body truncate max-w-[150px]">{inv.companies?.name}</p>
                          <p className="text-[9px] md:text-label font-bold text-slate-500">{inv.subscription_plans?.name} Plan</p>
                        </td>
                        <td className="px-8 py-6 md:py-8 font-black text-violet-600 text-body md:text-heading-3">${inv.amount}</td>
                        <td className="px-8 py-6 md:py-8">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
               <div className="bg-amber-50 dark:bg-amber-500/5 p-6 rounded-3xl border-2 border-amber-100 dark:border-amber-500/20">
                  ...
                  <p className="text-heading-1 font-black text-slate-900 dark:text-white">{alerts.filter(a => a.type === 'renewal_warning').length}</p>
               </div>
               <div className="bg-red-50 dark:bg-red-500/5 p-6 rounded-3xl border-2 border-red-100 dark:border-red-500/20">
                  ...
                  <p className="text-heading-1 font-black text-slate-900 dark:text-white">{alerts.filter(a => a.type === 'expired').length}</p>
               </div>
               <div className="bg-violet-50 dark:bg-violet-500/5 p-6 rounded-3xl border-2 border-violet-100 dark:border-violet-500/20">
                  ...
                  <p className="text-heading-1 font-black text-slate-900 dark:text-white">{alerts.filter(a => a.type === 'upgrade_suggestion').length}</p>
               </div>
            </div>
            ...
            <div className="space-y-4">
               {alerts.length === 0 ? (
                 <div className="py-20 text-center bg-slate-50 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200">
                    ...
                    <p className="font-black uppercase text-label text-slate-400 tracking-widest">System Clear: No Urgent Alerts</p>
                 </div>
                 ...
                 <div key={billingAlert.id} className="p-6 md:p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 hover:border-violet-500/30 transition-all">
                    <div className="flex items-center gap-6 text-center md:text-left">
                       ...
                       <div>
                          <h4 className="text-heading-3 font-black text-slate-900 dark:text-white uppercase tracking-tight">{billingAlert.companies?.name}</h4>
                          <p className="text-label font-bold text-slate-500 uppercase tracking-widest mt-1">{billingAlert.message}</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-3">
                       <button 
            ...
            {gateways.map(gw => (
              <div key={gw.id} className="p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-white/5 rounded-[3rem] flex flex-col justify-between group hover:border-violet-600/30 transition-all">
                <div className="space-y-6">
                  ...
                  <div>
                    <h4 className="text-heading-2 font-black text-slate-900 dark:text-white uppercase tracking-tight">{gw.display_name}</h4>
          ...
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
              <div className="p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 rounded-[3rem] shadow-sm relative overflow-hidden group">
                ...
                <h3 className="text-heading-1 font-black text-slate-900 dark:text-white mt-1">${metrics.totalRevenue.toLocaleString()}</h3>
              ...
              <div className="p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 rounded-[3rem] shadow-sm relative overflow-hidden">
                ...
                <h3 className="text-heading-1 font-black text-slate-900 dark:text-white mt-1">${metrics.mrr.toLocaleString()}</h3>
              ...
              <div className="p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 rounded-[3rem] shadow-sm relative overflow-hidden">
                ...
                <h3 className="text-heading-1 font-black text-slate-900 dark:text-white mt-1">{metrics.activeSubs}</h3>
              ...
              <div className="p-8 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 rounded-[3rem] shadow-sm relative overflow-hidden">
                ...
                <h3 className="text-heading-1 font-black text-slate-900 dark:text-white mt-1">{metrics.expiredSubs}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               ...
               <div className="lg:col-span-2 p-10 bg-slate-900 text-white rounded-[3rem] shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-10">
                     <div>
                        <h4 className="text-heading-2 font-black uppercase tracking-tighter">Revenue by Plan</h4>
                        <p className="text-label font-bold text-slate-400 uppercase tracking-widest mt-1">Income breakdown per tier</p>
                     </div>
                     ...
                     {metrics.planStats.map(stat => (
                        <div key={stat.name} className="space-y-3">
                           <div className="flex justify-between items-end">
                              <div>
                                 <span className="text-label font-black uppercase tracking-widest" style={{ color: stat.color }}>{stat.name}</span>
                                 <p className="text-body font-bold text-slate-400">{stat.count} Subscriptions</p>
                              </div>
                              <span className="text-heading-3 font-black">${stat.revenue.toLocaleString()}</span>
                           </div>
               ...
               <div className="p-10 bg-white dark:bg-white/5 border-2 border-slate-100 dark:border-violet-500/15 rounded-[3rem] flex flex-col justify-between">
                  <div>
                    <h4 className="text-heading-2 font-black text-slate-900 dark:text-white uppercase tracking-tighter">Growth Status</h4>
                  ...
                  <div className="flex-1 flex items-center justify-center py-10">
                     <div className="relative">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                           <span className="text-heading-1 font-black text-slate-900 dark:text-white">{Math.round((metrics.activeSubs / ((metrics.activeSubs + metrics.expiredSubs) || 1)) * 100)}%</span>
                  ...
                  <div className="space-y-4">
                     <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-2xl">
                        ...
                        <span className="text-body font-black text-slate-900 dark:text-white">$50,000</span>
                     </div>
